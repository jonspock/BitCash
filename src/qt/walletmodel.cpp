...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered bitcash address / amount:
            ...
            CRecipient recipient = {scriptPubKey, rcp.amount, rcp.fSubtractFeeFromAmount, rcp.referenceline.toUtf8().constData(),GetSecondPubKeyForDestination(DecodeDestination(rcp.address.toStdString()))};
            vecSend.push_back(recipient);
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal bitcash:URI (bitcash:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(EncodeDestinationHasSecondKey(address));
...
bool WalletModel::bumpFee(uint256 hash)
{
    ...
    questionString.append(BitcashUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(BitcashUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(BitcashUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
